...
void SceneViewportPanel::Draw(ImTextureID textureID)
{
    ...
    m_State.contentMin = ImGui::GetCursorScreenPos();
    m_State.contentMax = ImVec2(
        m_State.contentMin.x + static_cast<float>(m_State.width),
        m_State.contentMin.y + static_cast<float>(m_State.height)
    );
